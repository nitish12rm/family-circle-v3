...
export default function JoinView({ code }: { code: string }) {
  ...
  const handleJoin = async () => {
    ...
    setJoining(true);
    try {
      const res = await api.post<{ family: Family }>(`/api/join/${code}`, {});
      const families = await api.get<Family[]>("/api/families");
      setFamilies(families);
      setJoinedFamilyId(res.family?.id ?? info?.family?.id ?? null);
    } catch (e) {
  ...
  const finishJoin = () => router.replace("/tree");
  ...
  return (
    <>
          ...
          <div className="bg-bg-2 border border-border rounded-2xl p-6 shadow-card text-center">
            {loading ? (
              ...
              <>
                ...
                <p className="text-text-muted text-sm mt-1">Setting up your place in the tree…</p>
              </>
            ) : (
              <>
      ...
      {/* Tree placement modal — shown immediately after join */}
      {joinedFamilyId && (
        <PlaceMemberModal
          open={!!joinedFamilyId}
          familyId={joinedFamilyId}
          onComplete={finishJoin}
          onSkip={finishJoin}
        />
      )}
    </>
